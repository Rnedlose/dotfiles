...
fullscreen)
  ...
  # Get the focused output for multi-monitor setups
  focused_output=$(swaymsg -t get_outputs | jq -r '.[] | select(.focused) | .name')
  grim -o "$focused_output" "$file" # Fullscreen screenshot of focused monitor
  wl-copy < "$file" # Also copy to clipboard
  ;;
window)
...
area-clipboard)
  file="$HOME/Pictures/Screenshots/screenshot-$(date +'%d-%m-%Y_%H-%M-%S').png"
  grim -g "$(slurp)" "$file" # Area screenshot (save to file)
